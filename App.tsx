...
const ServerIcon: React.FC<{ active?: boolean; img?: string; label?: string; color?: string }> = ({ active, img, label, color }) => (
  <div className="group relative flex items-center justify-center w-[72px] mb-2 cursor-pointer">
    ...
    <div className={`w-12 h-12 rounded-[24px] group-hover:rounded-[16px] ${active ? 'rounded-[16px]' : ''} transition-all duration-200 overflow-hidden flex items-center justify-center text-white font-bold text-sm relative shadow-md ${color ? '' : 'bg-discord-server hover:bg-discord-accent'}`} style={{ backgroundColor: color }}>
...
const ChannelItem: React.FC<{ name: string; active?: boolean; type?: 'text' | 'voice' }> = ({ name, active, type = 'text' }) => (
  <div className={`flex items-center px-2 py-1.5 mx-2 rounded-md mb-0.5 cursor-pointer group transition-colors ${active ? 'bg-discord-active text-white' : 'text-discord-text-muted hover:bg-discord-active/50 hover:text-discord-text-normal'}`}>
    ...
    <span className={`font-medium text-sm truncate ${active ? 'text-white' : ''}`}>{type === 'text' && '#'} {name}</span>
  </div>
);
...
const App: React.FC = () => {
  ...
  const gridLayoutStyles: React.CSSProperties = {
    display: 'grid',
    gap: '16px',
    width: userCount === 1 ? 'min(100%, 1100px)' : '100%',
    height: userCount === 1 ? 'min(100%, 650px)' : '100%',
  ...
  return (
    <div className="flex h-screen w-screen bg-discord-base text-discord-text-normal font-sans overflow-hidden selection:bg-discord-accent selection:text-white">
      
      {/* Server List Sidebar - Uses custom-scrollbar for identical styling */}
      <nav className="w-[72px] bg-discord-server py-3 flex flex-col items-center shrink-0 overflow-y-auto custom-scrollbar z-30 shadow-md border-r border-black/10">
        <ServerIcon label="DM" color="#5865F2" />
        <div className="w-8 h-[2px] bg-discord-sidebar rounded-lg mb-2 opacity-50"></div>
        ...
        <div className="mt-auto flex flex-col gap-2">
           <div className="w-12 h-12 rounded-[24px] bg-discord-sidebar hover:bg-discord-green text-discord-green hover:text-white transition-all flex items-center justify-center cursor-pointer group">
      ...
      <div className="w-60 bg-discord-sidebar flex flex-col shrink-0 hidden md:flex rounded-tl-xl mt-0 z-20 border-r border-[#222]">
        <header className="h-12 border-b border-black/20 flex items-center justify-between px-4 hover:bg-white/5 cursor-pointer transition-colors shadow-sm">
          <h1 className="font-bold text-white truncate font-sans text-sm tracking-wide">Mierdicord</h1>
        ...
        <div className="flex-1 overflow-y-auto py-3 custom-scrollbar">
          ...
          <div className="px-2 mb-4">
            <div className="flex items-center justify-between px-2 mb-1 text-[11px] font-bold text-discord-text-muted uppercase hover:text-discord-text-normal cursor-pointer tracking-wider">
        ...
        <div className="h-[52px] bg-[#232428] px-2 flex items-center justify-between">
          <div className="flex items-center gap-2 hover:bg-white/5 p-1 rounded cursor-pointer group">
             <div className="w-8 h-8 rounded-full bg-discord-accent flex items-center justify-center text-white font-bold text-xs shadow-sm">You</div>
             <div className="text-xs">
                <div className="text-white font-bold group-hover:underline">You</div>
                <div className="text-discord-text-muted text-[10px]">Online</div>
             </div>
          </div>
          <div className="flex items-center gap-1">
             <button className="p-1.5 hover:bg-white/10 rounded text-gray-400 hover:text-gray-200 transition-colors"><svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg></button>
             <button className="p-1.5 hover:bg-white/10 rounded text-gray-400 hover:text-gray-200 transition-colors"><svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M19.14 12.94c.04-.3.06-.61.06-.94 0-.32-.02-.64-.07-.94l2.03-1.58c.18-.14.23-.41.12-.61l-1.92-3.32c-.12-.22-.37-.29-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94l-.36-2.54c-.04-.24-.24-.41-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96c-.22-.08-.47 0-.59.22L3.16 8.87c-.11.2-.06.47.12.61l2.03 1.58c-.05.3-.09.63-.09.94s.02.64.07.94l-2.03 1.58c-.18.14-.23.41-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.38 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.56 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32c.11-.22.06-.47-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6-3.6-1.62 3.6-3.6 3.6z"/></svg></button>
          </div>
        </div>
      </div>

      {/* Main Content Area */}
      <main className="flex-1 flex flex-col bg-discord-base relative min-w-0 shadow-[inset_10px_0_20px_-10px_rgba(0,0,0,0.5)]">
        <header className="h-12 border-b border-black/20 flex items-center px-4 shrink-0 shadow-sm bg-discord-base/95 backdrop-blur z-10">
           <div className="flex items-center gap-2 text-discord-text-normal font-bold text-base">
              ...
              <span className="text-xs text-discord-text-muted font-normal ml-2 px-2 border-l border-white/10">High Quality Voice</span>
           </div>
        </header>
        ...
        <div className="flex-1 p-4 overflow-hidden flex flex-col relative items-center justify-center">
          ...
          >
            {allUsers.map((user, index) => (
              <div 
                 key={user.uid} 
                 className={`relative w-full h-full overflow-hidden rounded-lg shadow-lg bg-black 
                  ${userCount === 1 ? 'max-w-5xl mx-auto aspect-video' : ''} 
                  ${userCount === 3 && index === 2 ? 'col-span-2 max-w-[50%] mx-auto w-full' : ''}`}
              >
                <MediaPlayer 
      ...
      <aside className="w-60 bg-discord-sidebar hidden lg:flex flex-col shrink-0 border-l border-black/10">
        ...
        <div className="flex-1 overflow-y-auto px-3 custom-scrollbar">
           {allUsers.map(user => (
              <div key={user.uid} className="flex items-center gap-3 p-2 rounded hover:bg-[#35373C] cursor-pointer group transition-colors mb-1">
                 <div className="relative">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm ${user.isLocal ? 'bg-discord-accent' : 'bg-discord-server'}`}>
                       {user.isLocal ? 'ME' : String(user.uid).slice(0,1).toUpperCase()}
                    </div>
                    <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-discord-sidebar rounded-full flex items-center justify-center">
                        <div className={`w-2.5 h-2.5 rounded-full border-2 border-discord-sidebar ${user.hasAudio ? 'bg-green-500' : 'bg-red-500'}`}></div>
                    </div>
                 </div>
                 <div className="min-w-0">
                    <div className={`text-sm font-medium truncate ${user.isLocal ? 'text-white' : 'text-discord-text-normal group-hover:text-white'}`}>
           ...
           {['Jane Doe', 'Moderator Bot', 'Admin'].map(name => (
              <div key={name} className="flex items-center gap-3 p-2 rounded hover:bg-[#35373C] cursor-pointer group opacity-50 grayscale hover:grayscale-0 transition-all mb-1">
